...
class CallManager private constructor(context: Context) : ITUINotification {
    ...
    fun call(
        ...
        TUICallEngine.createInstance(context)
            .call(userId, mediaType, params, object : TUICommonDefine.Callback {
    ...
    fun groupCall(
        ...
        Logger.i(
            TAG,
            "groupCall, groupId: $groupId, userIdList: $userIdList, mediaType: $mediaType, params: $params"
        )
        ...
        if (groupId.isNullOrEmpty()) {
            ...
            callback?.onError(
                ...
                "groupCall failed, groupId is empty"
            )
            return
        }
        ...
        if (list.isNullOrEmpty()) {
            ...
            callback?.onError(
                TUICallDefine.ERROR_PARAM_INVALID,
                "groupCall failed, userIdList is empty"
            )
            return
        }
    ...
    fun calls(
        ...
        TUICallEngine.createInstance(context)
            .calls(list, mediaType, params, object : TUICommonDefine.Callback {
                override fun onSuccess() {
                    ...
                    if (list.size >= 2 || (params != null && !params.chatGroupId.isNullOrEmpty())) {
                        scene = TUICallDefine.Scene.SINGLE_CALL
                    }
    ...
    fun joinInGroupCall(
        ...
        if (intRoomId <= 0 && strRoomId.isNullOrEmpty()) {
            ...
            callback?.onError(
                TUICallDefine.ERROR_PARAM_INVALID,
                "joinInGroupCall failed, roomId is invalid"
            )
            return
        }
        if (groupId.isNullOrEmpty()) {
            ...
            callback?.onError(
                TUICallDefine.ERROR_PARAM_INVALID,
                "joinInGroupCall failed, groupId is invalid"
            )
            return
        }
        if (TUICallDefine.MediaType.Unknown == mediaType) {
            ...
            callback?.onError(
                TUICallDefine.ERROR_PARAM_INVALID,
                "joinInGroupCall failed, mediaType is unknown"
            )
            return
        }
        userState.selfUser.get().id = TUILogin.getLoginUser() ?: ""
        ...
        TUICallEngine.createInstance(context).joinInGroupCall(
            roomId, groupId, mediaType,
            ...
            .inviteUser(userIdList, params, object : TUICommonDefine.ValueCallback<List<String>> {
                override fun onSuccess(data: List<String>?) {
                    if (data.isNullOrEmpty()) {
                        ...
                        callback?.onError(
                            TUICommonDefine.Error.FAILED.value,
                            "inviteUser failed, userList is empty"
                        )
                        return
                    }
            ...
            override fun onGranted() {
                TUICallEngine.createInstance(context)
                    .openCamera(
                        ...
                        object : TUICommonDefine.Callback {
                            override fun onSuccess() {
                                val status: TUICallDefine.Status =
                                    userState.selfUser.get().callStatus.get()
                            ...
                            override fun onError(errCode: Int, errMsg: String) {
                                Logger.e(
                                    TAG,
                                    "openCamera failed, errorCode: $errCode, errMsg: $errMsg"
                                )
                                callback?.onError(errCode, errMsg)
                            }
                        })
    ...
    fun startRemoteView(
        userId: String,
        videoView: VideoView?,
        callback: TUICommonDefine.PlayCallback?
    ) {
        Logger.i(TAG, "startRemoteView, userId: $userId, videoView: $videoView")
        TUICallEngine.createInstance(context)
            .startRemoteView(userId, videoView?.getVideoView(), callback)
    }
    ...
    fun setBlurBackground(enable: Boolean) {
        ...
        TUICallEngine.createInstance(context)
            .setBlurBackground(level, object : TUICommonDefine.Callback {
                override fun onSuccess() {
    ...
    private fun getCommonErrorMap(): Map<Int, String> {
        ...
        map[TUICallDefine.ERROR_PACKAGE_NOT_PURCHASED] =
            context.getString(R.string.tuicallkit_package_not_purchased)
        map[TUICallDefine.ERROR_PACKAGE_NOT_SUPPORTED] =
            context.getString(R.string.tuicallkit_package_not_support)
        map[TUICallDefine.ERROR_INIT_FAIL] =
            context.getString(R.string.tuicallkit_error_invalid_login)
        map[TUICallDefine.ERROR_PARAM_INVALID] =
            context.getString(R.string.tuicallkit_error_parameter_invalid)
        map[TUICallDefine.ERROR_REQUEST_REFUSED] =
            context.getString(R.string.tuicallkit_error_request_refused)
        map[TUICallDefine.ERROR_REQUEST_REPEATED] =
            context.getString(R.string.tuicallkit_error_request_repeated)
        map[TUICallDefine.ERROR_SCENE_NOT_SUPPORTED] =
            context.getString(R.string.tuicallkit_error_scene_not_support)
        return map
    }
